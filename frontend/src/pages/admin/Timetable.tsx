...
import { Calculator, BookOpen, FlaskConical, Globe, Palette, GraduationCap, Activity, Upload, Download } from 'lucide-react';
...
export default function AdminTimetable() {
    ...
    const fetchTimetable = useCallback(async () => {
        try {
            ...
            if (response.ok) {
                ...
                // No timetable found, that's okay
                setTimetable(null);
            }
        } catch (error) {
    ...
    const handleDownloadSample = async () => {
        try {
            const token = localStorage.getItem('token');
            const response = await fetch(`${API_BASE}/api/timetable/sample`, {
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
            });
            ...
            if (response.ok) {
                ...
                toast({
                    title: 'Success',
                    description: 'Sample timetable CSV downloaded',
                });
            }
        } catch (error) {
    ...
    const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            const token = localStorage.getItem('token');
            const response = await fetch(`${API_BASE}/api/timetable/upload`, {
                method: 'POST',
                headers: {
                    'Authorization': `Bearer ${token}`,
                },
            ...
            toast({
                title: 'Error',
                description: error instanceof Error ? error.message : 'Failed to upload timetable',
                variant: 'destructive',
            });
        } finally {
            setUploading(false);
    ...
    return (
        <div className="flex flex-col h-full p-6 bg-background">
            <div className="mb-6">
                <h2 className="text-3xl font-bold text-foreground mb-4">Timetable Management</h2>
                ...
                {timetable && (
                    <Card className="mb-4">
                        <CardHeader>
                            <CardTitle>Current Timetable Info</CardTitle>
                        </CardHeader>
                        <CardContent>
            ...
            {timetable ? (
                ...
                <Card>
                    <CardContent className="flex flex-col items-center justify-center py-12">
                        ...
                        <p className="text-muted-foreground mb-4">Upload a timetable to get started</p>
                    </CardContent>
                </Card>
            )}
        </div>
